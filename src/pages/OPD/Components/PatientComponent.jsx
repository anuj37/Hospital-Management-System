...
function Patient({
  ...
  if (patientObject) {
    patientObject.encounters.forEach((encounter) => {
      if (encounter.encounterType.display === ENCOUNTER_TYPE_VITALS) {
        ...
        if (
          latestVitalEncounter === null ||
          new Date(latestVitalEncounter.encounterDateTime) <
        ...
        item.setMembers.forEach((element) => {
          ...
          switch (conceptClass) {
            case CONCEPT_DIAGNOSIS: {
              selected = diagnosisObject.filter((obj) => {
            ...
            case CONCEPT_INVESTIGATION: {
              selected = investigationObject.filter((obj) => {
                return obj.uuid === element.uuid;
              })[0].selected;
              conceptClassFound = true;
              break;
            ...
            case CONCEPT_PROCEDURE: {
              selected = procedureObject.filter((obj) => {
                return obj.uuid === element.uuid;
              })[0].selected;
              conceptClassFound = true;
              break;
            ...
            case CONCEPT_SYMPTOPM: {
              selected = symptomsObject.filter((obj) => {
                return obj.uuid === element.uuid;
              })[0].selected;
              conceptClassFound = true;
              break;
  ...
  const handleInvestigationSelect = (investigations) => {
    let newInvestigations = genericSelect({
      theObject: investigationObject,
      selectedArray: investigations,
    });
  ...
  const handleInvestigationUnselect = (investigations) => {
    let newInvestigations = genericUnSelect({
      theObject: investigationObject,
      selectedArray: investigations,
    });
  ...
  const handleDiagnosisSelect = (diagnosis) => {
    let newDiagnosis = genericSelect({
      theObject: diagnosisObject,
      selectedArray: diagnosis,
    });
  ...
  const handleDiagnosisUnselect = (diagnosis) => {
    let newDiagnosis = genericUnSelect({
      theObject: diagnosisObject,
      selectedArray: diagnosis,
    });
  ...
  const handleSymptomSelect = (symptoms) => {
    let newSymptoms = genericSelect({
      theObject: symptomsObject,
      selectedArray: symptoms,
    });
  ...
  const handleSymptomUnselect = (symptoms) => {
    let newSymptoms = genericUnSelect({
      theObject: symptomsObject,
      selectedArray: symptoms,
    });
  ...
  const handleProcedureSelect = (procedures) => {
    let newProcedures = genericSelect({
      theObject: procedureObject,
      selectedArray: procedures,
    });
  ...
  const handleProcedureUnselect = (procedures) => {
    let newProcedures = genericUnSelect({
      theObject: procedureObject,
      selectedArray: procedures,
    });
          ...
          <Paper className={classes.root}>
            <GridContainer>
              ...
              <GridItem item xs container direction="column">
                <Typography variant="subtitle1" spacing={2}>
                  <b>Gender:</b>{" "}
                  {patientObject.person.gender === "M" ? "Male" : "Female"}
                </Typography>
              </GridItem>
            </GridContainer>
            <GridContainer>
          ...
          <TabPanel value={value} index={0}>
            ...
            >
              <DialogTitle id="vitalsDialogTitle">
                ...
                <Chip label={`${patientObject.person.display}`} />
              </DialogTitle>
                    ...
                    return (
                      <Grid item xs={12} sm={6} md={3} key={vital.uuid}>
                        <Tooltip title={vital.display}>
                          <TextField
                            InputProps={{ readOnly: !editVitals }}
                            id={vital.display}
                            label={vital.display}
                            value={value}
                          />
            ...
            <Button onClick={handleVitalDialogOpen} variant="contained">
              Vitals
            </Button>
            <br />
            <br />
            ...
            <TransferList
              ...
              handleUnselect={handleDiagnosisUnselect}
            ></TransferList>
            <br />
            <TransferList
              label="Procedure"
            ...
            />
            <br />
            <VisitOutcomeComponent
              setVisitOutcomeDetails={setVisitOutcomeDetails}
            ></VisitOutcomeComponent>
            <br />
            <PatientReferralComponent
              selectedVisit={selectedVisit}
              setPatientReferralDetails={setVisitOutcomeDetails}
            ></PatientReferralComponent>
            <br />
