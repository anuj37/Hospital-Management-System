...
function ProcedureInvestigationOrder(props) {
  ...
  const [servicdetailsVal, setServicdetailsVal] = React.useState(props.serviceDetailsprops.serviceDetails);
  ...
  const getNewaddInvestigationData = () =>{
    getAPI("/procedureinvestigationorder/patient?patient="+patientData.id+"&date="+patientData.orderdate)
    .then((response) => {
      const intestList = response.data.testOrderDetails[0].serviceDetailsForTestOrder[0].billableServiceDetails;
     
      console.log(intestList);
      setServicdetailsVal(intestList);
      
      const to = 
         intestList.reduce((sum, item) => {
          return sum + parseInt(item.price) * parseInt("1");
        }, 0)
        setTotal(to)
        setNetAmount(to);
        ...
        intestList.map((item) => {
              if(!unitpricetextfield.includes(item.price)){unitpricetextfield.push(item.price)}
              if(!quantitymultiplyprice.includes(item.price)){quantitymultiplyprice.push(item.price)}
              
            })
      
      const textfieldqunatity = 
      intestList.map((item) => {
          return "1";
        })
...
setOpen(false);
  ...
  const addNewInvestigation = (e,uuid) => {

    if(!newTests.includes(uuid)){
      newTests.push(uuid);
      checked.push(true);
      //setChecked(checked.length+1,true);
