...
export default function PatientRegistration() {
                  ...
                  return (
                    <React.Fragment key={uuid}>
                      <AutocompleteComponent
                        ...
                        validateAutocomplete={validateAutocomplete}
                      />
                      {formValues[display]?.datatype?.display === "Document" && (
                        <PatientConsent
  ...
  function changeConsentStatus(name,status){
    setPatientConsentStatus(patientConsentStatus.filter((item,index)=>{
      if(item.name === name){
        item.value = status;
      }
      return item;
  ...
  function onAutocompleteChange(display, newValue) {
    ...
    if(newValue.uuid === '3d16a441-949b-4aac-97f9-977c75a3cfe5'){
      ...
      newValue.answers.map((item) =>{
        if(parseInt(formValues["Age*"].slice(0, -1)) < 10 && item.uuid === '327f7959-0249-4182-aa02-e31be2dfed85'){
          newValue = item;
          return newValue;
        }
        if(parseInt(formValues["Age*"].slice(0, -1)) > 10 && item.uuid === '30dd68b3-955a-4396-b976-44566c8f8ec6'){
          newValue = item;
          return newValue;
        }
       })
